...
export function CodeRunner({ defaultLanguage = "python" }: CodeRunnerProps) {
  ...
  return (
    <section data-testid="code-runner" className="space-y-3 rounded-lg border bg-card p-4 shadow-sm">
      <header className="flex items-center justify-between">
        <h2 className="font-semibold text-card-foreground">Latihan kode</h2>
        <select
      ...
      <textarea
        ...
        className="min-h-[160px] w-full rounded-md border bg-muted p-3 font-mono text-sm text-foreground placeholder:text-muted-foreground"
      />
      ...
      {(mutation.data || mutation.isPending) && (
        <pre className="whitespace-pre-wrap rounded-md bg-slate-950 dark:bg-slate-900 p-3 text-xs text-slate-100 border">
